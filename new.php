...
<main class="container">
  ...
  <div class="card">
    <div class="code-display">
      ...
      <div class="meta-row">
        ...
        <?php else: ?>
        <span class="meta-pill">📝 <?= htmlspecialchars($createdShare['title'] ?? 'Text') ?></span>
  ...
  <div class="card">
    <div class="result-header">
      <h3><?= $share['type'] === 'text' ? '<span style="color:var(--muted);font-weight:500;font-size:0.85rem;">Title:</span> ' . htmlspecialchars($share['title'] ?? 'Untitled') : 'Here\'s your file' ?></h3>
  ...
  <div style="text-align:center; margin-top: 8px;">
    <a href="?action=retrieve" style="color: var(--muted); font-size: 0.88rem; text-decoration: none;">← Retrieve another</a>
  </div>
  ...
  <!-- Tabs -->
  <?php $defaultTab = ($action === 'retrieve' && !$share) ? 'retrieve' : 'share'; ?>
  <div class="tabs">
    <button class="tab-btn <?= $defaultTab === 'share' ? 'active' : '' ?>" onclick="switchTab('share', this)">Share</button>
    <button class="tab-btn <?= $defaultTab === 'retrieve' ? 'active' : '' ?>" onclick="switchTab('retrieve', this)">Retrieve</button>
  </div>

  <!-- Share pane -->
  <div id="sharePane" <?= $defaultTab === 'retrieve' ? 'class="hidden"' : '' ?>>
    <div class="card">
      <!-- Type switcher -->
      ...
      <form method="POST" id="textForm">
        ...
        <div class="field">
          ...
          <input type="text" name="title" id="textTitle" placeholder="e.g. My code snippet, Meeting notes…" maxlength="80" value="<?= htmlspecialchars($_POST['title'] ?? '') ?>">
        </div>
        <div class="field">
